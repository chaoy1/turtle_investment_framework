...
def download_annual_report(url, save_path, max_retries=DEFAULT_MAX_RETRIES):
    ...
    for attempt in range(1, max_retries + 1):
        try:
            ...
            # Windows does not allow deleting an open file. Validate inside
            # the context manager, then remove the temporary file after close.
            if invalid_magic:
                os.remove(tmp_path)
                return (
                    False,
...
def print_result(success, filepath="", filesize=0, url="", stock_code="",
    ...
    """Print structured result block for the LLM agent to parse."""
