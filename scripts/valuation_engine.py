...
class ValuationEngine:
    ...
    Requires a MarketDataClient with populated ``_store``.
    """
    ...
    def _basic_info(self) -> dict:
        ...
        # New provider-neutral schema. Values are already normalized to
        # millions and raw shares by market_collector.py.
        standardized_cap = self._sf(row.get("market_cap_mm"))
        standardized_shares = self._sf(row.get("shares"))
        if standardized_cap is not None:
            return {
                "name": row.get("name"),
                "close": close,
                "market_cap_mm": standardized_cap,
                "total_shares": standardized_shares,
            }
    ...
    def compute_wacc(self) -> dict:
        ...
        # The provider-neutral collector normalizes balance-sheet values.
        ...
        if not cf_df.empty and total_shares and total_shares > 0:
            ...
            for year in list(yearly.keys()):
                ...
                if not div_paid or div_paid <= 0:
                    continue
                # Data providers commonly report div_paid as a positive outflow.
...
def main():
    ...
    from market_collector import MarketDataClient
