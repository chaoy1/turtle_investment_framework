...
# Run at the start of each Codex/GPT agent session on macOS, Linux, or WSL
...
# 3. Load optional environment settings
...
echo "  Market data: AKShare + Yahoo Finance (no data token required)"
if [ -z "$OPENAI_API_KEY" ]; then
    echo "  INFO: OPENAI_API_KEY not set"
    echo "  Option 1: cp .env.sample .env && edit .env"
    echo "  Option 2: export OPENAI_API_KEY='your_key_here'"
    echo "  This is expected when running through an authenticated Codex session"
else
    echo "  OPENAI_API_KEY: set (${#OPENAI_API_KEY} chars)"
    echo "  GPT model: ${OPENAI_MODEL:-gpt-5.6}"
    echo "  Reasoning effort: ${OPENAI_REASONING_EFFORT:-high}"
fi
...
$PYTHON_BIN -m pytest tests/ -x -q --tb=short --basetemp .test-tmp/pytest-setup 2>&1 | tail -5

echo ""
echo "=== Setup complete ==="
echo "To run: python scripts/market_collector.py --code 600887.SH --output output/data_pack_market.md"
