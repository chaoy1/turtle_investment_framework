...
3. 若用户只给了公司名称没给代码，通过交易所或可信公开检索确认代码
...
2. `{output_dir}/data_pack_market.md` — 公开结构化数据包
   - 若不存在 → **停止执行**，同上提示
...
│   数据源 = AKShare + Yahoo Finance（无需 Token）   │
└──────────┬──────────────────────────────────────┘
           │
           ▼
┌─────────────────────────────────────────────────┐
...
┌─────────────────────────────────────────────────┐
│  Step A: 市场数据刷新                              │
│  market_collector.py --refresh-market            │
│  → 更新 §1/§2/§11；保留原财务章节                     │
│  → 双源失败时保留原文件                                │
└──────────┬──────────────────────────────────────┘
           │
...
```
# === 使用 --refresh-market 模式更新市场章节 ===
# 若 AKShare 与 Yahoo Finance 同时失败，脚本保留原文件

Bash(
  command = "python3 scripts/market_collector.py --code {ts_code} --output {output_dir}/data_pack_market.md --refresh-market",
...
| 公开数据获取失败 | --refresh-market 降级使用 yfinance fallback |
...
├── scripts/                                    ← 预处理脚本（只读）
│   ├── market_collector.py                    ← 数据采集脚本（支持 --refresh-market）
...
所有阶段（Phase 1/2/3）的金额统一为 **百万元**（公开数据源报表原值 ÷ 1e6）。
