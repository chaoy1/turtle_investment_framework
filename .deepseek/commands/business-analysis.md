...
**1A: AKShare / Yahoo Finance structured data**
```bash
mkdir -p output/{code}_{company}
python3 scripts/market_collector.py --code $ARGUMENTS --output output/{code}_{company}/data_pack_market.md
```
...
若运行时支持子代理（如 DeepSeek TUI），启动一个 general-purpose 子代理执行；否则由当前 Agent 直接执行。任务提示词：
...
Data files:
    - 公开数据源 data: output/{code}_{company}/data_pack_market.md
...
若运行时支持子代理则启动子代理执行（否则由当前 Agent 直接执行）。任务提示词：
...
## v2 Key Changes
- **PDF-first architecture**: Annual report PDF is the primary data source; 公开数据源 provides supplementary historical series
...
- If 公开数据源 fails → use yfinance fallback
- If PDF and 公开数据源 data conflict → trust PDF, note discrepancy
